...
namespace Editor.Huatuo.ThirdPart.ICSharpCode.SharpZipLib.Zip
{
	...
	public class FastZip
	{
		...
		public IEnumerator ExtractZip(string zipFileName, string targetDirectory, string fileFilter)
		{
			var itor = ExtractZip(zipFileName, targetDirectory, Overwrite.Always, null, fileFilter, null, restoreDateTimeOnExtract_);
			while (itor.MoveNext())
            {
		...
		public IEnumerator ExtractZip(string zipFileName, string targetDirectory,
							   Overwrite overwrite, ConfirmOverwriteDelegate confirmDelegate,
							   string fileFilter, string directoryFilter, bool restoreDateTime)
		{
			Stream inputStream = File.Open(zipFileName, FileMode.Open, FileAccess.Read, FileShare.Read);
			var itor = ExtractZip(inputStream, targetDirectory, overwrite, confirmDelegate, fileFilter,
				directoryFilter, restoreDateTime, true);
			while (itor.MoveNext())
			{
				yield return itor.Current;
			}
		...
		public IEnumerator ExtractZip(Stream inputStream, string targetDirectory,
					   Overwrite overwrite, ConfirmOverwriteDelegate confirmDelegate,
					   string fileFilter, string directoryFilter, bool restoreDateTime,
					   bool isStreamOwner)
		{
		...
		private Action<int> OnBegin;
		private Action<int> OnProgress;
		private Action OnFailure, OnComplete;

		public static IEnumerator ExtractZip(string zipFileName, string targetDir,
			Action<int> begin, Action<int> progress, Action complete, Action failure)
		{
			var zip = new FastZip()
			{
				OnBegin = begin,
			...
			var itor = zip.ExtractZip(zipFileName, targetDir, "");
			while (itor.MoveNext())
            {
				yield return itor.Current;
            }
		...
		public static bool CreateZip(string zipFileName, string srcDir)
        {
			var zip = new FastZip();
			zip.CreateZip(zipFileName, srcDir, true, ".*");
